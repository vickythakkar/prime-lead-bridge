...
export default function AdminBilling() {
  ...
  async function handleApplyDiscount(invoice, discountAmount) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        fetchData(); // Sync live summary stats
      } else {
  ...
  function handleDownload(invoice) {
    ...
    const planName = invoice.organizations?.subscription_plan 
      ? invoice.organizations.subscription_plan.replace(/_/g, ' ').toUpperCase()
      : 'PAY AS YOU GO';
