      ...
      <div className="glass-card rounded-2xl overflow-hidden">
        {loading ? (
          ...
          <div className="divide-y divide-white/5">
            {voicemails.map(vm => (
              <div key={vm.id} className={`p-5 transition-colors ${vm.status === 'new' ? 'bg-indigo-500/5' : 'hover:bg-white/5'}`}>
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex items-center gap-4">
                    {vm.status === 'new' ? (
                      <div className="bg-indigo-500 rounded-full h-3 w-3 shrink-0 shadow-[0_0_8px_rgba(99,102,241,0.8)]"></div>
                    ) : (
                      <div className="h-3 w-3 rounded-full bg-slate-700 shrink-0"></div>
                    )}
                    <div>
                      <h4 className="font-bold text-white">{vm.from_number}</h4>
                      <div className="flex items-center gap-3 text-xs text-slate-400 mt-0.5">
                        <span>{new Date(vm.created_at).toLocaleString()}</span>
                        {vm.duration > 0 && <><span>•</span><span>{vm.duration}s</span></>}
                        <span>•</span>
                        <span className="text-indigo-400">{vm.organizations?.company_name || vm.organizations?.name || 'Unknown Org'}</span>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="w-60">
                      {vm.audio_link ? (
                        <audio
                          ...
                          className="h-9 w-full [&::-webkit-media-controls-panel]:bg-slate-800 [&::-webkit-media-controls-current-time-display]:text-white [&::-webkit-media-controls-time-remaining-display]:text-white"
                        />
                      ) : (
