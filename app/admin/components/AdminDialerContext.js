...
export function AdminDialerProvider({ children }) {
  ...
  useEffect(() => {
    async function setupDevice() {
      try {
        ...
        setStatus('Ready to Call');
        newDevice.register().catch(e => console.warn('Registration failed:', e));
  ...
  const showFloatingBar = activeCall && pathname !== '/admin/dialer';
        ...
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 animate-in slide-in-from-bottom-10 fade-in duration-300">
          <div className="glass-card bg-slate-900/90 backdrop-blur-xl border border-white/10 shadow-2xl rounded-full px-6 py-3 flex items-center gap-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <Link href="/admin/dialer" className="h-10 w-10 rounded-full bg-slate-800 text-slate-300 hover:text-white hover:bg-slate-700 flex items-center justify-center transition-all" title="Return to Dialer">
