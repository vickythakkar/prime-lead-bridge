...
export default function AdminCallWrapUpModal({ isOpen, onClose, callDetails, initialData }) {
  ...
  async function handleRemoveDisposition() {
    if (!form.disposition) return;
    if (!confirm(`Are you sure you want to remove "${form.disposition}" from the list of outcomes?`)) return;
    
    const updated = dispositions.filter(d => d !== form.disposition);
    setDispositions(updated);
    setForm({ ...form, disposition: '' });
  }
          ...
          <div>
            ...
            {!isAddingDisposition ? (
              <div className="flex gap-2">
                ...
                {form.disposition && (
                  <button type="button" onClick={handleRemoveDisposition} title="Remove this outcome" className="px-3 py-2 bg-rose-500/10 border border-rose-500/20 rounded-lg text-rose-400 hover:bg-rose-500/20 hover:text-rose-300 transition-colors">
