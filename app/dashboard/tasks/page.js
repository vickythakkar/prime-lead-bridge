...
export default function TasksPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      if (agentData) {
        ...
        await fetchTasks(agentData.organization_id, filter);
      }
    }
    init();
  }, []);

  async function fetchTasks(oId, status) {
    const targetOrgId = oId || orgId;
  ...
  useEffect(() => {
    if (orgId) fetchTasks(orgId, filter);
  }, [filter]);

  useEffect(() => {
    const handleUpdate = () => { if (orgId) fetchTasks(orgId, filter); };
  ...
  return (
    <div className="animate-in fade-in duration-500">
      <header className="mb-8 flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-white">Follow-ups & Tasks</h1>
          <p className="text-slate-400 mt-1">Manage all your scheduled follow-ups and reminders.</p>
        </div>
      </header>
            ...
            return (
              <div key={task.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-slate-900/50 border border-white/5 rounded-xl hover:border-white/10 transition-colors gap-4">
                ...
                <div className="flex items-center gap-2 sm:self-start">
                  ...
                  {task.phone_number && (
                    <Link href={`/dashboard/dialer?phone=${encodeURIComponent(task.phone_number)}`} className="w-10 h-10 ml-2 rounded-lg bg-emerald-500/10 text-emerald-400 flex items-center justify-center hover:bg-emerald-500/20 transition-colors" title="Call Now">
      ...
      <EditTaskModal
        isOpen={!!editingTask}
        onClose={() => { setEditingTask(null); fetchTasks(orgId, filter); }}
        task={editingTask}
      />
    </div>
  );
